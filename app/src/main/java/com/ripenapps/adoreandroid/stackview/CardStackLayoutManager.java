...
import com.ripenapps.adoreandroid.stackview.internal.CardStackSetting;
import com.ripenapps.adoreandroid.stackview.internal.CardStackSmoothScroller;
import com.ripenapps.adoreandroid.stackview.internal.CardStackState;
import com.ripenapps.adoreandroid.stackview.internal.DisplayUtil;
    ...
    private void update(RecyclerView.Recycler recycler) {
        ...
        if (state.isSwipeCompleted()) {
            ...
            new Handler().post(new Runnable() {
                @Override
                public void run() {
                    listener.onCardSwiped(direction, state.topPosition);
                    View topView = getTopView();
                    if (topView != null) {
                        listener.onCardAppeared(getTopView(), state.topPosition);
                    }
        ...
        if (state.status.isDragging()) {
            listener.onCardDragging(state.getDirection(), state.getRatio(), state.topPosition);
        }
    }
